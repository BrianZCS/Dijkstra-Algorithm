...
// TA: Keren
// Lecturer: Gary
// Notes to Grader: add one more method (getTarget(T source)) to help backend.java
...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  /**
   * Find all possible target of all edge connected with start point given the start point
   * 
   * @return all targets which connect to the source by edge
   */
  public List<T> getTarget(T source) {
    ...
    if (sourceVertex == null)
      throw new IllegalArgumentException(
          "Cannot retrieve weight of edge between vertices that do not exist");
    for (int i = 0; i < sourceVertex.edgesLeaving.size(); ++i)
      targets.add(sourceVertex.edgesLeaving.get(i).target.data);
    return targets;
  }
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    ...
    if (start == null || end == null)
      throw new NoSuchElementException("Cannot find shortest path for null data");
    Vertex startVertex = vertices.get(start);
    Vertex endVertex = vertices.get(end);
    // vertex is null
    if (startVertex == null || endVertex == null)
      throw new NoSuchElementException(
          "Cannot find shortest path between vertices that do not exist");
    Hashtable<T, Vertex> selectedVertices = new Hashtable<>();
    PriorityQueue<Path> pathToFrontier = new PriorityQueue<>();
    Path path = new Path(startVertex);
    pathToFrontier.add(path);
    while (pathToFrontier.size() > 0) {
      path = pathToFrontier.remove();
      // if two path have the same frontier, After finding the shortest path, so skip the other path
      // because the end has been selected
      if (selectedVertices.contains(path.end))
        continue;
      Vertex selectedVertex = path.end;
      if (selectedVertex == endVertex)
        return path;
      selectedVertices.put(selectedVertex.data, selectedVertex);
      for (Edge edge : selectedVertex.edgesLeaving) {
        // Do not add the path to selected vertex
        if (!selectedVertices.contains(edge.target))
          pathToFrontier.add(new Path(path, edge));
      }
    }
    // no shortest path found
    throw new NoSuchElementException("Cannot find shortest path between the two vertices");
  }
